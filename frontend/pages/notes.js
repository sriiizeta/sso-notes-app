    ...
    <div style={{ maxWidth: 700, margin: '60px auto', padding: '30px 40px', borderRadius: 20, boxShadow: '0 0 10px rgba(0,0,0,0.1)', background: '#fdfdfd', fontFamily: 'Segoe UI, sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
        <input
          ...
          style={{ flex: 1, padding: '10px 14px', border: '1px solid #ddd', borderRadius: 12, outline: 'none', fontSize: 15, backgroundColor: '#fffafc' }}
        />
        <button
          onClick={add}
          disabled={loading}
          style={{ backgroundColor: '#a3d8f4', border: 'none', padding: '10px 18px', borderRadius: 20, cursor: 'pointer', transition: '0.2s', fontWeight: '600', color: '#333', opacity: loading ? 0.6 : 1 }}
        ...
        <a
          ...
          style={{ textDecoration: 'none', padding: '10px 18px', borderRadius: 20, backgroundColor: '#ffd6a5', color: '#333', fontWeight: '600' }}
        >
          Logout
        </a>
      </div>
      ...
      <div>
        {notes.length === 0 ? (
          ...
          <ul style={{ listStyleType: 'disc', paddingLeft: 20 }}>
            {notes.map((n) => (
              <li key={n._id} style={{ background: '#f0f7f4', padding: '12px 15px', borderRadius: 14, marginBottom: 10, boxShadow: '0 1px 4px rgba(0,0,0,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontSize: 16, color: '#333' }}>{n.text}</div>
                  <div style={{ fontSize: 12, color: '#777', marginTop: 4 }}>{new Date(n.createdAt).toLocaleString()}</div>
                </div>
                <button
                  onClick={() => remove(n._id)}
                  style={{ backgroundColor: '#ffadad', border: 'none', padding: '6px 12px', borderRadius: 14, cursor: 'pointer', transition: '0.2s', fontWeight: '600' }}
